...
export async function evaluateEventForPublication(eventId:string):Promise<PublicationResult>{
  const now=new Date();
  const event=await prisma.event.findUniqueOrThrow({where:{id:eventId},include:{sources:{include:{source:true,rawEvent:true}},organizer:true,category:true,occurrences:{where:{startsAt:{gte:now}},select:{startsAt:true},take:1}}});
  ...
  const structuredCinema=trustedCinema&&(event.category.slug==="cinema"||Boolean(cinemaCategory))&&event.occurrences.length>0&&Boolean(event.imageUrl)&&Boolean(event.venueId||event.address);
